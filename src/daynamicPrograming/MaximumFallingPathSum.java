...
public class MaximumFallingPathSum {
    ...
    void solveMemoizationIterative(int n, int m, int[][] matrix, int[][] dp) {
        ...
        for (int i = 1; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                dp[n][m] = Math.max(straight, Math.max(leftDiagonal, rightDiagonal));
            }
        }
    }
    int solveMemoizationIterativeOptimize(int n, int m, int[][] matrix) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int j = 0; j < m; j++) {
                ...
                cur[j] = Math.max(straight, Math.max(leftDiagonal, rightDiagonal));
            }
            prev=cur;
        }
    ...
    int solveMemoization(int n, int m, int[][] matrix, int[][] dp) {
        ...
        return dp[n][m] = Math.max(straight, Math.max(leftDiagonal, rightDiagonal));
    }
